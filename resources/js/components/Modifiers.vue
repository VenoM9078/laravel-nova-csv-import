<template>
  <div class="space-y-4">
    ...
    <p>
      Use modifiers to modify the value of the source selected above
      <i>before</i> it gets saved to your resource. Modifiers are combinatory
      meaning you can stack them together to do weird and wonderful things with
      your data. They are executed in the order defined.
    </p>

    <p><b>TIP</b>: You can drag and drop modifiers to re-order them.</p>

    <draggable v-model="modifiers" handle=".handle" item-key="modifier">
        ...
        <div class="border-rounded bg-gray-100 p-2 handle mb-2">
          <div class="flex space-x-2 items-start">
            ...
            <div class="flex flex-col space-y-2">
              <select
                @change="(event) => (element.name = event.target.value)"
                :value="element.name"
                class="form-control form-select form-select-bordered"
              >
                <option value="">- Do not modify -</option>
                <option v-for="mod in mods" :key="mod.name" :value="mod.name">
                  {{ mod.title }}
                </option>
              </select>
              ...
              <div v-if="mods[element.name]?.settings">
                ...
                >
                  ...
                  <select
                    ...
                    @change="
                      (event) => (element.settings[name] = event.target.value)
                    "
